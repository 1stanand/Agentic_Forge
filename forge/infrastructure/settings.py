...
from forge.shared import paths
...
FEATURES_REPO_PATH = os.getenv("FEATURES_REPO_PATH", "").strip()
...
DOMAIN_KNOWLEDGE_PATH = str(paths.DOMAIN_KNOWLEDGE_TOML_PATH)
PLANNER_HINTS_PATH = str(paths.PLANNER_HINTS_TOML_PATH)
ASSEMBLER_HINTS_PATH = str(paths.ASSEMBLER_HINTS_TOML_PATH)
...
LLM_CHAT_FORMAT = os.getenv("LLM_CHAT_FORMAT", "").strip()
LLM_BACKEND = os.getenv("LLM_BACKEND", "llama_cpp").strip().lower()
LLM_API_BASE_URL = os.getenv("LLM_API_BASE_URL", "http://127.0.0.1:8080/v1").strip()
LLM_API_MODEL = os.getenv("LLM_API_MODEL", "").strip()
