...
from casforge.retrieval.embedder import load_index, search_index
from casforge.workflow.ordering import detect_stage, detect_sub_tags, STAGE_TAGS, SUB_TAGS
from casforge.retrieval.query_expander import (
    expand_for_vector,
    expand_for_fts,
    expand_for_trigram,
    normalise_query_text,
)
from casforge.shared.paths import RETRIEVAL_SETTINGS_TOML_PATH

# â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€
# Channel weights
# â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€

# Phase 3: Load retrieval settings from TOML config
def _load_retrieval_config() -> dict:
    """Load retrieval settings from TOML, with safe defaults."""
    defaults = {
        "channel_weights": {"vector": 0.50, "fts": 0.30, "trigram": 0.20},
        "trigram_min_similarity": 0.10,
        "stage_boost": 1.30,
        "stage_hint_boost": 1.60,
        "rerank_fetch_multiplier": 5,
        "min_rerank_fetch": 20,
        "stopwords": [
            "a", "an", "and", "any", "all", "at", "be", "both", "button", "by",
            "for", "from", "if", "in", "into", "is", "it", "level", "of", "on",
            "or", "screen", "should", "stage", "that", "the", "to", "user", "when", "with"
        ],
    }

    try:
        if RETRIEVAL_SETTINGS_TOML_PATH.exists():
            config = tomllib.loads(RETRIEVAL_SETTINGS_TOML_PATH.read_text(encoding="utf-8"))
            logger.info("Loaded retrieval settings from %s", RETRIEVAL_SETTINGS_TOML_PATH)
            return config
    except Exception as exc:
        logger.warning("Failed to load retrieval settings TOML: %s — using defaults", exc)

    return defaults

_RETRIEVAL_CONFIG = _load_retrieval_config()

_WEIGHTS = _RETRIEVAL_CONFIG.get("channel_weights", {"vector": 0.50, "fts": 0.30, "trigram": 0.20})
_TRIGRAM_MIN = _RETRIEVAL_CONFIG.get("trigram_min_similarity", 0.10)
_STAGE_BOOST = _RETRIEVAL_CONFIG.get("stage_boost", 1.30)
_STAGE_HINT_BOOST = _RETRIEVAL_CONFIG.get("stage_hint_boost", 1.60)
_RERANK_FETCH_MULTIPLIER = _RETRIEVAL_CONFIG.get("rerank_fetch_multiplier", 5)
_MIN_RERANK_FETCH = _RETRIEVAL_CONFIG.get("min_rerank_fetch", 20)
_STOPWORDS = set(_RETRIEVAL_CONFIG.get("stopwords", []))
...
def _channel_trigram(
    ...
    # SELECT word_similarity(%s, step_text)  â†’ query
    # WHERE  word_similarity(%s, step_text)  â†’ query
    #        > %s                            â†’ _TRIGRAM_MIN
...
def _merge(
    ...
    Returns merged hits plus per-channel score details for reranking/debugging.
    """
    ...
    details = {
        sid: {
            "vector": v_scores.get(sid, 0.0),
            "fts": f_scores.get(sid, 0.0),
            "trigram": t_scores.get(sid, 0.0),
            "merged_pre_rerank": merged[sid],
        }
        for sid in all_ids
    }
    return sorted(merged.items(), key=lambda x: -x[1]), details


# â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€
# Stage + sub-stage detection and boost
# (stage names and annotation tags come from workflow/ordering.py â†’ order.json)
# â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€
...
def _overlap_ratio(terms: list[str], text: str) -> float:
    if not terms:
        return 0.0
    haystack = f" {str(text or '').lower()} "
    hits = sum(1 for term in terms if f" {term} " in haystack or term in haystack)
    return hits / max(1, len(terms))


def _scope_overlap(terms: list[str], ctx: dict) -> float:
    scope_parts = []
    scope_parts.extend(ctx.get("scope_stage_tags") or [])
    scope_parts.extend(ctx.get("scope_sub_tags") or [])
    scope_parts.extend(ctx.get("scope_product_types") or [])
    scope_parts.extend(ctx.get("scope_application_stages") or [])
    return _overlap_ratio(terms, " ".join(scope_parts))


def _scenario_key(ctx: dict) -> tuple:
    return (
        ctx.get("file_path") or "",
        ctx.get("scenario_id") or 0,
        ctx.get("scenario_title") or "",
    )
...
def _rerank_results(
    ...
    scenario_scores: dict[tuple, float] = {}
    scenario_notes: dict[tuple, list[str]] = defaultdict(list)

    for step_id, merged_score in merged_hits:
        ctx = context_map.get(step_id)
        if not ctx:
            continue
        scenario_key = _scenario_key(ctx)
        step_text = ctx.get("step_text", "")
        title = ctx.get("scenario_title", "")
        scope_bonus = 0.14 * _scope_overlap(terms, ctx)
        title_bonus = 0.18 * _overlap_ratio(terms, title)
        overlap_bonus = 0.22 * _overlap_ratio(terms, step_text)
        screen_bonus = 0.10 if screen_filter and (ctx.get("screen_context") or "").lower() == screen_filter.lower() else 0.0

        mismatch_penalty = 0.0
        candidate_stages = {s.lower() for s in (ctx.get("scope_stage_tags") or [])}
        if detected_stage and candidate_stages and detected_stage.lower() not in candidate_stages:
            mismatch_penalty += 0.18
            scenario_notes[scenario_key].append("cross-stage penalty")

        candidate_subs = {s.lower() for s in (ctx.get("scope_sub_tags") or [])}
        if detected_sub_tags and candidate_subs and not candidate_subs.intersection({s.lower() for s in detected_sub_tags}):
            mismatch_penalty += 0.08
            scenario_notes[scenario_key].append("sub-tag mismatch")

        scenario_score = merged_score + scope_bonus + title_bonus + screen_bonus - mismatch_penalty
        if scenario_score > scenario_scores.get(scenario_key, float("-inf")):
            scenario_scores[scenario_key] = scenario_score

        detail = score_details.setdefault(step_id, {})
        detail["scenario_score"] = scenario_score
        detail["overlap_bonus"] = overlap_bonus
        detail["title_bonus"] = title_bonus
        detail["scope_bonus"] = scope_bonus + screen_bonus
        detail["mismatch_penalty"] = mismatch_penalty
    ...
    for step_id, merged_score in merged_hits:
        ...
        detail = score_details.setdefault(step_id, {})
        step_score = (
            scenario_scores.get(scenario_key, merged_score)
            + detail.get("overlap_bonus", 0.0)
            + (0.04 if ctx.get("keyword") == "Then" and any(t in query.lower() for t in ("verify", "validate", "display", "show")) else 0.0)
            + (0.03 if ctx.get("keyword") == "When" and any(t in query.lower() for t in ("click", "select", "enter", "submit", "remove", "add")) else 0.0)
        )
        detail["final_score"] = step_score
        reranked.append((step_id, step_score))

    reranked.sort(key=lambda item: -item[1])

    results: list[dict] = []
    seen_pairs: set[tuple] = set()
    for step_id, final_score in reranked:
        ctx = context_map.get(step_id)
        if not ctx:
            continue
        dedupe_key = (_scenario_key(ctx), ctx.get("step_text", "").strip().lower())
        if dedupe_key in seen_pairs:
            continue
        seen_pairs.add(dedupe_key)
        notes = scenario_notes.get(_scenario_key(ctx), [])
        results.append({
            **ctx,
            "score": round(max(0.0, final_score), 4),
            "ranking_notes": ", ".join(dict.fromkeys(notes)),
            "score_details": {
                **score_details.get(step_id, {}),
                "scenario_score": score_details.get(step_id, {}).get("scenario_score", 0.0),
                "overlap_bonus": score_details.get(step_id, {}).get("overlap_bonus", 0.0),
                "title_bonus": score_details.get(step_id, {}).get("title_bonus", 0.0),
                "scope_bonus": score_details.get(step_id, {}).get("scope_bonus", 0.0),
                "mismatch_penalty": score_details.get(step_id, {}).get("mismatch_penalty", 0.0),
            },
        })
...
def search(
    ...
    Args:
        ...
        stage_hint:     Plain-text stage name supplied by the caller (e.g. "Recommendation").
                        Used for a stronger boost (1.6x) even when the intent text itself
                        does not contain the stage name.  Detected stage from query text
                        still applies (1.3x), upgraded to 1.6x when they match.
    ...
    conn = get_conn()
    try:
        # â"€â"€ Run all three channels â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€
        ...
        # â"€â"€ Merge scores â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€
        merged, score_details = _merge(vector_hits, fts_hits, trigram_hits)
        ...
        # -- Stage boost -------------------------------------------------------
        # Auto-detected stage (from query text): 1.3x boost.
        # Explicit stage_hint from caller: 1.6x boost (upgraded from 1.3x when
        # hint matches auto-detected; additive when they differ).
        hint_tag: Optional[str] = None
        if stage_hint:
            hint_tag = detect_stage(stage_hint)
            if not hint_tag:
                # stage_hint may already be a @Tag string
                hint_tag = stage_hint if stage_hint.startswith("@") else None

        if detected_stage:
            boost = _STAGE_HINT_BOOST if hint_tag == detected_stage else _STAGE_BOOST
            ...
            if stage_ids:
                ...
                merged = _apply_boost(merged, stage_ids, boost)
                all_candidate_ids = [sid for sid, _ in merged]

        if hint_tag and hint_tag != detected_stage:
            # Caller specified a stage not present in the intent text — boost separately
            hint_ids = _fetch_tagged_step_ids(conn, [hint_tag], all_candidate_ids)
            if hint_ids:
                logger.debug("Stage hint boost (%s) -> %d candidates", hint_tag, len(hint_ids))
                merged = _apply_boost(merged, hint_ids, _STAGE_HINT_BOOST)
                all_candidate_ids = [sid for sid, _ in merged]

        # â"€â"€ Sub-stage boost (Guarantor, Primary, etc.) â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€
        ...
        if detected_sub_tags:
            ...
            if sub_ids:
                logger.debug("Sub-tag boost %s â†’ %d candidates", detected_sub_tags, len(sub_ids))
                merged = _apply_boost(merged, sub_ids, 1.15)

        # â"€â"€ Keyword post-filter â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€
        ...
        # Fetch enough context for a second-pass rerank using scenario/title/scope signals.
        rerank_fetch = max(top_k * _RERANK_FETCH_MULTIPLIER, _MIN_RERANK_FETCH)
        top_ids_with_scores = merged[:rerank_fetch]
        top_ids = [sid for sid, _ in top_ids_with_scores]

        # â"€â"€ Fetch context for all top steps â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€
    ...
    reranked_results = _rerank_results(
        merged_hits=top_ids_with_scores,
    ...
    for row in reranked_results:
        row["detected_stage"] = detected_stage
        row["detected_sub_tags"] = detected_sub_tags
